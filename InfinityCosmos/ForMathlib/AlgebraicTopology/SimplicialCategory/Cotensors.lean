...
@[blueprint
  ...
  (statement := /-- Let $\cA$ be a simplicial category. The \textbf{cotensor} of an object $A \in
    \cA$ by a simplicial set $U$ is given by the data of an object $A^U \in \cA$ together with a
    cone $U \to \cA(A^U,A)$ so that the induced map
...
@[blueprint
  ...
  (statement := /-- Assuming such objects exist, the simplicial cotensor defines a bifunctor
    \begin{center}
